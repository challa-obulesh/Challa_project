...
def render_pixel_level(frame_bgr, label_map, obstacle_mask, nav, traffic_lights, boards):
    ...
    if obstacle_mask.any():
        if _OBS_COLOR_IMG is None or _OBS_COLOR_IMG.shape != out.shape:
            _OBS_COLOR_IMG = np.full(out.shape, C_OBSTACLE, dtype=np.uint8)
        blended = cv2.addWeighted(out, 0.45, _OBS_COLOR_IMG, 0.55, 0)
        cv2.copyTo(blended, obstacle_mask.astype(np.uint8), out)
...
def main():
    ...
    if fps_accum:
        ...
        with open("benchmark_v5.json", "w") as f:
            json.dump(benchmark, f, indent=2)
        print("[INFO] Benchmark saved to benchmark_v5.json")
